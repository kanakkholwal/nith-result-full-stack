...
// Class Rank  (Increasing Order)
Branches.forEach((branch) => {
    Batches.forEach((batch) => {
        const correspondingClassRanks = ranking(data.filter(({ Batch, Branch }) => Batch === batch && Branch === branch), (a, b) => parseFloat(b.semesters[b.semesters.length - 1].cgpi) > parseFloat(a.semesters[a.semesters.length - 1].cgpi));
        data.filter(({ Batch }) => Batch === batch).forEach((stud, index) => {
            stud.Class_Rank = correspondingClassRanks[index];
        });
    });
});
